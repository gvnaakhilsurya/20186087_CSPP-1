...
def hangman(secret_word):
    ...
    * After each round, you should also display to the user the
    ...
    while True:
        ...
        if char_entered not in list_1:
            if is_word_guessed(secret_word, char_entered):
                print("Good guess: " + get_guessed_word(secret_word,\
                    entered_string_by_user.split()))
            else:
                print("Oops! That letter is not in my word :" + get_guessed_word(secret_word,\
                 entered_string_by_user.split()))
                wrong_guess_count -= 1
        else:
            print("Oops! That letter is already given : " +get_guessed_word\
                (secret_word, entered_string_by_user.split()))
        list_1 = list_1 + entered_string_by_user.split()
        if get_guessed_word(secret_word, char_entered) == secret_word:
